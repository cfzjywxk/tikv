...
#[test]
fn test_campaign_req_sent_to_replica() {
    init_log_for_test();
    let mut cluster = new_server_cluster(0, 3);
    let base_tick_ms = 50;
    cluster.cfg.raft_store.raft_base_tick_interval = ReadableDuration::millis(base_tick_ms);
    ...
    // Wait until all peers of region 1 hibernate and then stop peer 2.
    info!("[for debug] begin to wait until region is hibernated");
    thread::sleep(Duration::from_millis(base_tick_ms * 30));
    info!("[for debug] finished to wait until region is hibernated");

    // Result:
    // test_campaign_req_sent_to_replica 2024/07/05 18:00:35.830 peer.rs:5764:
    // [DEBG] failed to propose, err: peer is not leader for region 1, leader
    // may Some(id: 2 store_id: 2), message: header { region_id: 1 peer { id: 1
    // store_id: 1 } region_epoch { conf_ver: 3 version: 1 } } requests {
    // cmd_type: Put put { cf: "default" key: 6B31 value: 6B31 } }, peer_id: 1,
    // region_id: 1, thread_id: 161b&^
    //
    // test_campaign_req_sent_to_replica 2024/07/05 18:00:36.234 raft.rs:1556:
    // [INFO] starting a new election, term: 7, raft_id: 1, region_id: 1, thread_id:
    // 161
    let res = put_with_timeout(&mut cluster, 1, b"k1", b"k1", Duration::from_secs(1));
    info!("[for debug] put on node=2 res={:?}", res);

    thread::sleep(Duration::from_millis(base_tick_ms * 100));
    info!("[for debug] test is finished");
    // cluster.stop_node(2);
    //
    // // Peer 3 will:
    // // 1. steps a heartbeat message from its leader and then ticks 1 time.
    // // 2. ticks a peer_stale_state_check, which will change state from Idle
    // to // PreChaos. 3. continues to tick until it hibernates totally.
    // let (tx, rx) = mpsc::sync_channel(128);
    // fail::cfg_callback("on_raft_base_tick_idle", move ||
    // tx.send(0).unwrap()).unwrap(); let mut raft_msg =
    // RaftMessage::default(); raft_msg.region_id = 1;
    // raft_msg.set_from_peer(new_peer(1, 1));
    // raft_msg.set_to_peer(new_peer(3, 3));
    // raft_msg.mut_region_epoch().version = 1;
    // raft_msg.mut_region_epoch().conf_ver = 3;
    ...
    //
    // // Wait until the peer 3 hibernates again.
    // // Until here, peer 3 will be like `election_elapsed=3 &&
    // missing_ticks=6`. thread::sleep(Duration::from_millis(base_tick_ms *
    // 10));
    //
    // // Restart the peer 2 and it will broadcast `MsgRequestPreVote` later,
    // which // will wake up peer 1 and 3.
    // let (tx, rx) = mpsc::sync_channel(128);
    // let filter = RegionPacketFilter::new(1, 3)
    //     .direction(Direction::Send)
    //     .msg_type(MessageType::MsgRequestVote)
    ...
    // cluster.run_node(2).unwrap();
    //
    // // Peer 3 shouldn't start a new election, otherwise the leader may step
    // down // incorrectly.
    // rx.recv_timeout(Duration::from_secs(2)).unwrap_err();
}
